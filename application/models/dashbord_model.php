...
//       $query = $this->db->get();
	...
	function searchAdvance( $from, $to ) {
		...
		$sql   = "SELECT item.item, item.id as itemID, transaction.id, transaction.weight, transaction.nug, 
transaction.transaction_type, transaction.comment, transaction.created from item 
RIGHT JOIN transaction ON transaction.item_id = item.id WHERE transaction.created $condition ";
		$query = $this->db->query( $sql );
		$sno   = 1;
		foreach ( $query->result() as $row ) {
			$created = strtotime( $row->created );
